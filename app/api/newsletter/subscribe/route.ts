...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (existing) {
      ...
      await sendEmail({
        to: normalizedEmail,
        subject: "Confirm your subscription to Closed Companies",
        html: `<div style="font-family:Arial,Helvetica,sans-serif;line-height:1.5;color:#111"><h2>Confirm your subscription</h2><p>Click the button below to confirm your subscription to Closed Companies.</p><p><a href="${confirmUrl}" style="display:inline-block;padding:10px 16px;background:#000;color:#fff;text-decoration:none;border-radius:8px">Confirm subscription</a></p><p>If the button doesn't work, copy and paste this URL:<br/>${confirmUrl}</p></div>`,
      });
      return NextResponse.json({ ok: true });
    }
